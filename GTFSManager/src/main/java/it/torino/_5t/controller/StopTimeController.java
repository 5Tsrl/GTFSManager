...
//		}
		...
		model.addAttribute("listaFermateNonAssociate", stops);
	...
	public String submitStopTimeForm(@ModelAttribute @Valid StopTime stopTime, BindingResult bindingResult, @RequestParam("arrival") String arrivalTime, @RequestParam("departure") String departureTime, @RequestParam("stopId") Integer stopId, Model model, RedirectAttributes redirectAttributes, HttpSession session) {
		...
		Time arrival = new Time(Integer.parseInt(arrivalT[0]), Integer.parseInt(arrivalT[1]), Integer.parseInt(arrivalT[2]));
		stopTime.setArrivalTime(arrival);
		String[] departureT = departureTime.split(":");
		Time departure = new Time(Integer.parseInt(departureT[0]), Integer.parseInt(departureT[1]), Integer.parseInt(departureT[2]));
					...
					if (t.equals(trip)) {
						for (StopTime st: t.getStopTimes()) {
							if (st.getId().equals(stopTimeId)) {
								...
								session.setAttribute("corsaAttiva", t);
								session.setAttribute("lineaAttiva", r);
								logger.info("Fermata " + activeStopTime.getStop().getGtfsId() + " della corsa " + t.getGtfsId() + " aggiornata.");
								break;
							}
						}
						break;
			...
			if (r.getId().equals(route.getId())) {
				...
				for (Trip t: r.getTrips()) {
					if (t.equals(trip)) {
						...
						session.setAttribute("corsaAttiva", t);
						session.setAttribute("lineaAttiva", r);
						logger.info("Fermata " + stopTime.getStop().getGtfsId() + " eliminata dalla corsa " + t.getGtfsId());
						break;
					}
				}
				break;
